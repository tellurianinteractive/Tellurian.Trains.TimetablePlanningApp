...
public static class TrainPatternExtenstions
{
    public static Train CreateTrain(this TrainPattern pattern, string number, TimeSpan startTime)
    {
        ...
        foreach (var patternCall in pattern.Calls)
        {
            if (result.Calls.Any())
            {
                ...
                result.Add(new StationCall(patternCall.Track, new(previous.Departure.Time.Add(patternCall.Arrival.Time)), new(previous.Departure.Time.Add(patternCall.Departure.Time))));
            }
            else
            {
                result.Add(new StationCall(patternCall.Track, new(patternCall.Arrival.Time.Add(startTime)), new(patternCall.Departure.Time.Add(startTime))));
            }
        }
        return result;
    }
    ...
    public static TrainPattern SubSection(this TrainPattern pattern, string name,  int startIndex, int count)
    {
        var subPattern =  pattern.Calls.Skip(startIndex).Take(count).ToArray();
        ...
        return new TrainPattern(name, pattern.Colour) { Calls = subPattern};
    }
